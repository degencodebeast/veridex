...
def test_result_module_never_imports_directional_scorer():
    import inspect
    src = inspect.getsource(result_mod)
    assert "veridex.scoring" not in src        # no import of the directional scorer module
    assert "import scoring" not in src          # no `from veridex import scoring` / `import scoring`
    assert "report.leaderboard" not in src      # doesn't touch the directional leaderboard
