...
def test_envelope_ms_recv_ts_and_extra_forbid():
    ...
    with pytest.raises(ValidationError):  # extra="forbid" rejects unknown field
        RecorderHeartbeatEvent(
            sequence_no=1,
            event_type="RecorderHeartbeatEvent",
            source_ts=None,
...
def _fv(**kw):
    base = {
        "sequence_no": 1, "event_type": "FairValueEvent", "source_ts": 100, "recv_ts": 100000,
    ...
    base.update(kw)
    return FairValueEvent(**base)


def test_fair_value_missing_message_id_is_unavailable_lowercase():
    e = _fv()
    assert e.proof_status == "unavailable_no_message_id"
    with pytest.raises(ValidationError):
        _fv(proof_status="PROVEN")  # uppercase rejected
    with pytest.raises(ValidationError):
        _fv(message_id=None, proof_status="proven")  # no msg_id but claims proven → reject
    with pytest.raises(ValidationError):
        _fv(fv=1.4)  # decimal/out-of-range rejected
...
def _snap(**kw):
    base = {
        "sequence_no": 3, "event_type": "VenueBookSnapshotEvent", "source_ts": None, "recv_ts": 200000,
    ...
    base.update(kw)
    return VenueBookSnapshotEvent(**base)
...
def test_book_snapshot_stores_price_size_levels_not_mid():
    ...
    with pytest.raises(ValidationError):
        _snap(mid=0.61)  # mid-only summary rejected (extra=forbid)
    with pytest.raises(ValidationError):
        _snap(bids=({"price": 1.4, "size": 8.0},))  # price out of [0,1] rejected


def _ex(**kw):
    base = {
        "candidate_price": 0.60, "available_size_at_price": 8.0, "cumulative_size_to_clear": 5.0,
...
def test_executability_label_is_counterfactual_only_and_no_fill():
    e = _ex()
    assert e.label == "COUNTERFACTUAL"
    with pytest.raises(ValidationError):
        _ex(label="FILLED")  # only COUNTERFACTUAL allowed
    with pytest.raises(ValidationError):
        _ex(fill_price=0.6)  # no fill field
    with pytest.raises(ValidationError):
        _ex(real_executable_edge_bps=12)  # no executable edge
...
def _qi(**kw):
    ...
    base.update(kw)
    return QuoteIntentEvent(**base)


def test_quote_intent_has_no_post_decision_fields():
    e = _qi()
    assert e.queue_ahead_size == 8.0
    with pytest.raises(ValidationError):
        _qi(outbid_within_ms=200)  # post-decision field forbidden on the immutable intent
    with pytest.raises(ValidationError):
        _qi(stepped_ahead_count=3)  # post-decision field forbidden
    with pytest.raises(ValidationError):
        _qi(fill_price=0.6)  # no fill field


def _nq(**kw):
    base = {
        "sequence_no": 4, "event_type": "NoQuoteIntentEvent", "source_ts": None, "recv_ts": 109000,
        "decision_id": "d-109", "no_quote_reason": "stale",
    }
    base.update(kw)
    return NoQuoteIntentEvent(**base)


def test_no_quote_reason_is_closed_set():
    e = _nq()
    assert e.no_quote_reason == "stale"
    with pytest.raises(ValidationError):
        _nq(no_quote_reason="because_i_said_so")  # out-of-set reason rejected
    with pytest.raises(ValidationError):
        _nq(action="cancel")  # no fabricated action field
    with pytest.raises(ValidationError):
        _nq(fill_price=0.6)  # no fill field


def _decision(**kw):
    base = {
        "sequence_no": 5, "event_type": "DecisionEvent", "source_ts": None, "recv_ts": 110000,
        ...
        "config_hash": "cfg-hash", "policy_hash": "pol-hash", "model_inputs_hash": "mi-hash",
    }
    base.update(kw)
...
def test_decision_event_requires_null_source_ts():
    e = _decision()
    assert e.source_ts is None  # locally generated: source_ts must be null (REQ-015)
    with pytest.raises(ValidationError):
        _decision(source_ts=100)  # non-null source_ts on a DecisionEvent rejected


def test_event_type_must_match_class_name():
    e = _fv()
    assert e.event_type == "FairValueEvent"
    with pytest.raises(ValidationError):
        _fv(event_type="NoQuoteIntentEvent")  # discriminator mismatch rejected
